...
export const getTodosOfUser = async (userId: string): Promise<IResponseType<ITodo, 'todos'>> => {
    const {data} = await axiosInstance.get<IResponseType<ITodo, 'todos'>>(`user/${userId}/todos`);
    return data;
}
